...
public:
  ...
  bool write_traj_files = false; // flag to write data files for each individual
                                 // trajectory, will take up a lot of space
  ...
  int n_micro_steps = 1000; // number of Metropolis steps to use for averaging
                            // on the micro scale

  // int num_batches;        	            // number of batches to use for
  // microscale steady-state averaging in KMC_TTS double delta; // accuracy
  // level of microscale averaging, delta << 1
};
