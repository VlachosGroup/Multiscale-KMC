...
void KMC_traj_TTS ::simulate_micro() { // Implement with analytical solution for
                                       // A->B for now

  double r_accept; // random mumber between 0 and 1 used to accept or reject the
                   // reaction
  ...
  */
  ...
  // Should we scale the number of micro steps by the number of pairs of
  // reversible steps?
  for (int micro_step = 0; micro_step < in_data.n_micro_steps; micro_step++) {

    // Generate random numbers
    // fast_rxn_randi = micro_step % in_data.n_fast_rxns;      // alternates
    // between which reaction to attempt
    fast_rxn_randi =
        rand() %
        in_data.n_fast_rxns; // picks a random index for a fast reaction
    ...
    for (int i = 0; i < in_data.n_rxns; i++) {
      ...
      for (int j = 0; j < in_data.n_specs; j++) {
        if (in_data.stoich_mat[i][j] <
          ...
          micro_props[i] =
              micro_props[i] * pow(N[j], -in_data.stoich_mat[i][j]);
        }
      }
          ...
          for (int j = 0; j < in_data.n_specs; j++) {
            if (in_data.stoich_mat[i][j] <
                0) { // if this species is a reactant, use it to compute the
                     // rate
              micro_prop_ders[i][k] =
                  micro_prop_ders[i][k] * pow(N[j], -in_data.stoich_mat[i][j]);
            }
          }
    ...
    for (int j = 0; j < in_data.n_specs; j++) {
      if (in_data.stoich_mat[rev_rxn_to_try][j] <
          0) { // if this species is a reactant, use it to compute the rate
        a_rev =
            a_rev * pow(N_candidate[j], -in_data.stoich_mat[rev_rxn_to_try][j]);
      }
    }
    ...
    for (int j = 0; j < in_data.n_specs; j++) {
      if (in_data.stoich_mat[rev_rxn_to_try][j] <
          0) { // if this species is a reactant, use it to compute the rate
        rev_prop_ders[rev_rxn_to_try] =
            rev_prop_ders[rev_rxn_to_try] *
            pow(N_candidate[j], -in_data.stoich_mat[rev_rxn_to_try][j]);
      }
    }
    ...
    q_cum[micro_step] =
        Q; // sum of little partition functions up until that point
    ...
    if (if_accept) { // Metropolis criterion
      ...
      for (int i = 0; i < in_data.n_params; i++) {
        ...
        dEdth[i] += -1 / a_fwd * micro_prop_ders[fast_rxn_to_try][i] +
                    1 / a_rev * rev_prop_ders[i];
      }
    }
  }
  ...
  */
  ...
  for (int i = 0; i < in_data.n_specs; i++) {
    for (int j = 0; j < in_data.n_params; j++) {
      micro_scale_sens[i][j] = micro_scale_sens[i][j] / in_data.n_micro_steps -
                               N_micro_avg[i] * -1 * dEdth_avg[j];
    }
  }
  ...
  for (int i = 0; i < in_data.n_rxns; i++) {
    for (int j = 0; j < in_data.n_params; j++) {
      prop_ders_indirect[i][j] =
          prop_ders_indirect[i][j] / in_data.n_micro_steps -
          props[i] * -1 * dEdth_avg[j];
    }
  }
  ...
  */
  ...
  for (int i = 0; i < in_data.n_slow_rxns; i++) {
    slow_props_cum[i] =
        slow_props_cum[i] / slow_props_cum[in_data.n_slow_rxns - 1];
  }
