...
private:
  ...
  vector<vector<double>>
      prop_ders_direct; // Direct averaging of derviativeson the microscale
  vector<vector<double>>
      prop_ders_indirect; // Indirect averaging on the microscale
  vector<double> slow_props;
  vector<double> slow_props_cum;

public:
  vector<vector<vector<double>>>
      micro_scale_sens_profile; // 3-D vector of microscale sensitivities
