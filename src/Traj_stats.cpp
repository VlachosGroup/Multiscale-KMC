...
void Traj_stats ::finalize_stats() {
  ...
  */
  ...
  for (int i = 0; i < in_data.N_record; ++i) {
    ...
    for (int j = 0; j < in_data.n_specs; j++) {
      spec_profiles_averages[i][j] =
          spec_profiles_averages[i][j] / in_data.N_traj;
  ...
  for (int i = 0; i < in_data.N_record; ++i) {
    ...
    for (int j = 0; j < in_data.n_specs; j++) {
      ...
      for (int k = 0; k < in_data.n_params; k++) {
        ...
        sensitivities[i][j][k] -=
            spec_profiles_averages[i][j] * traj_deriv_avgs[i][k];
        sensitivities[i][j][k] =
            sensitivities[i][j][k] * in_data.N_traj / (in_data.N_traj - 1);
        // cout << sensitivities[i][j][k] << endl;
      }
    }
  }
  ...
  for (int traj_ind = 0; traj_ind < in_data.N_traj; traj_ind++) {
    ...
    for (int i = 0; i < in_data.N_record; ++i) {
      ...
      for (int j = 0; j < in_data.n_specs; j++) {
        ...
        for (int k = 0; k < in_data.n_params;
             k++) { // For TTS, add extra contribution from microscale averaging
          sensitivities[i][j][k] +=
              run->spec_profile[i][j] * run->traj_deriv_profile[i][k] +
              run->get_micro_scale_sens_profile(i, j, k);
          if (not std::isfinite(sensitivities[i][j][k])) {
            cout << traj_ind << " has NaNs" << endl;
            cout << in_data.rand_seed + traj_ind << " is the random seed."
                 << endl;
          }
        }
      }
...
void Traj_stats ::run_simulations() { // add some if statements which will
                                      // change bahavior depending on if it is
                                      // STS or TTS
  ...
  */
  ...
  in_data.N_traj =
      Npp * p; // round up to the nearest multiple of the number of processors
  ...
  */
  ...
  MPI_Gather(spec_profiles_averages_arr, size1, MPI_DOUBLE,
             spec_profiles_averages_big_arr, size1, MPI_DOUBLE, 0,
             MPI_COMM_WORLD);
  MPI_Gather(traj_deriv_avgs_arr, size2, MPI_DOUBLE, traj_deriv_avgs_big_arr,
             size2, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Gather(sensitivities_arr, size3, MPI_DOUBLE, sensitivities_big_arr, size3,
...
void Traj_stats ::write_spec_avg_output() {
  ...
  for (int time_ind = 0; time_ind < in_data.N_record; time_ind++) {
    ...
    for (int spec_ind = 0; spec_ind < in_data.n_specs; spec_ind++) {
      writer_spec_avgs
          << spec_profiles_averages[time_ind][spec_ind]
          << "\t"; // print the mean population of this species at this time
    }
...
void Traj_stats ::write_sensitivity_output() {
  ...
  // Write header
  writer_sensitivities
      << "Sensitivities for each species and rate constant versus time" << endl
      << endl;
    ...
    for (int time_ind = 0; time_ind < in_data.N_record; time_ind++) {
      ...
      for (int param_ind = 0; param_ind < in_data.n_params; param_ind++) {
        writer_sensitivities << sensitivities[time_ind][i][param_ind] *
                                    in_data.rate_const[param_ind]
                             << "\t"; // normalize by rate constant value
      }
