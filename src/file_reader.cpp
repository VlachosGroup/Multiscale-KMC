...
// Helper function which removes white spaces from the beginning and end of a
// string
string trim_string(const string &string_in) { // remove white spaces from the
...
file_reader ::file_reader(string flname) {
  ...
  if (myfile.is_open()) {
    ...
    while (getline(myfile, line)) {
      ...
      if (line == "Number of species") {
        ...
        for (int rxn_ind = 0; rxn_ind < n_rxns; rxn_ind++) {
          ...
          rate_const[rxn_ind] = atof(line.c_str());
                 ...
                 "write trajectory data") { // Flag to print out data for
                                            // individual trajectories

        write_traj_files = true;

      } else if (line ==
                 "two time scale") { // Flag to turn two time scale mode on

        two_time_scale = true;

      } else if (line ==
                 "Number of fast reaction pairs") { // Flag to turn two time
                                                    // scale mode on
        ...
        for (int i = 0; i < n_fast_rxns; i++) {
          fast_pairs[i].resize(2);
                 ...
                 "Fast reaction pairs") { // Flag to turn two time scale mode on
        ...
        for (int i = 0; i < n_rxns; i++) {
          ...
          if (find(begin(fast_rxns), end(fast_rxns), i) ==
              end(fast_rxns)) { // If it is not a fast reaction, it is a slow
                                // reaction
      ...
      } else { // Unrecognized command. Throw an error. Should also tell them
               // the line number
